...
function SearchContent() {
  ...
  // Fetch Discovery Data for empty search (uses limit: 80 which is already cached in memory/sessionStorage from homepage)
  useEffect(() => {
    const fetchDiscovery = async () => {
      if (searchQuery.trim() !== "" || activeTab !== "All") return;
      try {
        setDiscoveryLoading(true);
        // Matches the homepage cache key (limit: 10) -> 0 extra network requests
        const projectsRes = await projectsApi
          .list({ limit: 12 })
          .catch(() => []);
        const rawLatest = extractArray(projectsRes);

        // Latest for us (first 4 items)
        setLatestProjects(rawLatest.slice(0, 8));
      } catch (err) {
  ...
  // Fetch & Filter Search Results (debounced after user finishes typing)
  useEffect(() => {
    const fetchResults = async () => {
      ...
      try {
        ...
        const [projectsRes, devData] = await Promise.all([
          projectsApi.list({ limit: 12 }).catch(() => []),
          developersApi.list().catch(() => []),
        ]);

        const projectsList = extractArray(projectsRes);
        ...
        // 2. Filter Developers
        const matchingDevelopers: DeveloperItem[] = developersList.filter(
          (d) => {
        ...
        projectsList.forEach(
          (p) => p.location && areaSet.add(formatLocationName(p.location)),
        );
        developersList.forEach(
          (d) => d.location && areaSet.add(formatLocationName(d.location)),
        );

        const matchingAreas: AreaItem[] = Array.from(areaSet)
          .filter(
            (areaName) => !query || areaName.toLowerCase().includes(query),
          )
    ...
    // 400ms debounce: waits until user stops typing before running search
  ...
  const hasAnyResults =
    (activeTab === "All" &&
      (projects.length > 0 || developers.length > 0 || areas.length > 0)) ||
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <main className="pt-24 md:pt-28 pb-16 max-w-[1600px] mx-auto px-4 min-h-[80vh]">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <h1 className="text-2xl md:text-3xl font-bold flex-1 text-center pr-10">
            Search
          </h1>
        </div>
        ...
        <div className="flex gap-3 justify-center overflow-x-auto pb-4 scrollbar-hide mb-10 max-w-3xl mx-auto">
          {(["All", "Projects", "Developers", "Areas"] as TabType[]).map(
            (tab) => (
              <button
                key={tab}
